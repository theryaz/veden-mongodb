...
function debug(){
	if(this.debug === true) debug(arguments);
}
...
class MongoDatabase{
	...
	async getClient(database='default'){
		...
		// debug("Clients",Object.keys(this.clients));
	...
	*/
	async runTransaction(options,transactionFn){
		...
		async function attemptTransaction(client,session,attempt=0){
			if(attempt >= retries) throw new Error(`Transaction failed all ${attempt} tries`,"Errors: " + JSON.strinify(errors));
			debug("Transaction Attempt #",attempt);
	...
	getProjectionFromFields(params){
		...
		// debug("Using projection",projection);
		return projection;
	}
	...
	async getCursor(db_params, query={}){
		...
		let timeout = db_params.timeout || 10;
