...
class MongoDatabase{
	constructor(opts={}){
		...
		this.mongo_client = async (database) => {
			...
			if(connection === undefined){
				...
				let url = `mongodb://${AUTH_STRING?AUTH_STRING:''}${this.options.DB_HOSTS}/${database}?`;
				...
				let client = await MongoClient.connect(url,{"reconnectTries":100,reconnectInterval:2000});
	...
	async getCursor(db_params, query={}){
		...
		return db.collection(db_params.collection).find(query, options);
	}
	...
	async find(db_params, query={}){
		...
		if (db_params.from >= 1){
			...
			let db = await this.mongo_client(db_params.database);
			return db.collection(db_params.collection).find(query,{fields:projection}).sort(sort).limit(limit).skip(skip).toArray();
		}else{
			let db = await this.mongo_client(db_params.database);
			return db.collection(db_params.collection).find(query,{fields:projection}).sort(sort).limit(limit).toArray();
		}
	...
	async findOne(db_params, query={}){
		...
		if(db_params.collection === undefined) throw new Error("Collection not provided");
		let db = await this.mongo_client(db_params.database);
		return db.collection(db_params.collection).findOne(query,{fields:projection});
	}
	...
	async replaceOne(db_params, query={}, doc){
		...
		let db = await this.mongo_client(db_params.database);
		let result = await db.collection(db_params.collection).replaceOne(query,body,{upsert: upsert});
		return {
	...
	async findAndModify(db_params, query={}, doc){
		...
		let db = await this.mongo_client(db_params.database);
		return db.collection(db_params.collection).findAndModify(query,sort,doc);
	}
}
