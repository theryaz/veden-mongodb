...
class MongoDatabase{
 ...
 getProjectionFromFields(params){
	...
	async getCursor(db_params, query={}){
		...
		return db.collection(db_params.collection).find(query);
	}
	...
	async upsertOne(db_params, query={}, doc){
		...
		let db = await this.mongo_client(db_params.database);
		let insert_result = await db.collection(db_params.collection).update(query,body,{upsert: true});
		return {
			matched: insert_result.matchedCount,
			modified: insert_result.modifiedCount,
			upserted: insert_result.upsertedCount,
			upserted_id: insert_result.upsertedId,
			result: insert_result.result
		};
	}
	async updateOne(db_params, query={}, doc){
		...
		let db = await this.mongo_client(db_params.database);
		let insert_result = await db.collection(db_params.collection).updateOne(query,body);
		return {
			matched: insert_result.matchedCount,
			modified: insert_result.modifiedCount,
			result: insert_result.result
	...
	async updateMany(db_params, query={}, doc){
		...
		let db = await this.mongo_client(db_params.database);
		let insert_result = await db.collection(db_params.collection).updateMany(query,body);
		return {
			matched: insert_result.matchedCount,
			modified: insert_result.modifiedCount,
			result: insert_result.result
	...
	async deleteOne(db_params, query={}){
		...
		if(db_params.collection === undefined) throw new Error("Collection not provided");
		let db = await this.mongo_client(db_params.database);
		let insert_result = await db.collection(db_params.collection).deleteOne(query);
		return {
			deleted: insert_result.deletedCount
		};
	}
	async deleteMany(db_params, query={}){
		...
		if(db_params.collection === undefined) throw new Error("Collection not provided");
		let db = await this.mongo_client(db_params.database);
		let insert_result = await db.collection(db_params.collection).deleteMany(query);
		return {
			deleted: insert_result.deletedCount
		};
	}
