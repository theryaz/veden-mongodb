...
class MongoDatabase{
	constructor(opts={}){
		...
		this.options = {
			...
			DB_SSL : opts.DB_SSL,
			DB_RETRY_WRITES : opts.DB_RETRY_WRITES !== undefined ? true : false,
			DB_USE_SRV : opts.DB_USE_SRV !== undefined ? true : false,
			DEBUG : opts.DEBUG,
		};
	...
	getConnectionUrl(){
		...
		if(this.options.DB_USE_SRV === true){
			url = `mongodb+srv://${AUTH_STRING?AUTH_STRING:''}${this.options.DB_HOSTS}/?`;
			if(this.options.DB_RETRY_WRITES) url += `&retryWrites=true`;
		}else{
			url = `mongodb://${AUTH_STRING?AUTH_STRING:''}${this.options.DB_HOSTS}/?`;
	...
	*/
	...
	async insertMany(db_params, docs){
		...
		let db = await this.mongoClient(db_params.database);
		let insert_result = await db.collection(db_params.collection).insertMany(docs,{session: db_params.session, ordered: db_params.ordered});
		return {
	...
	async aggregate(db_params, pipeline){
		...
		if(db_params.collection === undefined) throw new Error("Collection not provided");
		let db = await this.mongoClient(db_params.database);
		let cursor = await db.collection(db_params.collection).aggregate(pipeline,{session: db_params.session});
		if(db_params.cursor === true){
			return cursor;
		}else{
	...
	async findOneAndUpdate(db_params, query={}, doc){
		if(db_params.database === undefined) throw new Error("Database not provided");
		if(db_params.collection === undefined) throw new Error("Collection not provided");
		let body = doc;
		if(body === undefined)
		...
		let db = await this.mongoClient(db_params.database);
		let result = await db.collection(db_params.collection).findOneAndUpdate(query,body,{session: db_params.session});
		return result;
	}
	async findOneAndReplace(db_params, query={}, doc){
		if(db_params.database === undefined) throw new Error("Database not provided");
		if(db_params.collection === undefined) throw new Error("Collection not provided");
		let body = doc;
		if(body === undefined)
		...
		let db = await this.mongoClient(db_params.database);
		let result = await db.collection(db_params.collection).findOneAndReplace(query,body,{session: db_params.session});
		return result;
	}
	async findOneAndDelete(db_params, query={}, doc){
		if(db_params.database === undefined) throw new Error("Database not provided");
		if(db_params.collection === undefined) throw new Error("Collection not provided");

		let db = await this.mongoClient(db_params.database);
		let result = await db.collection(db_params.collection).findOneAndDelete(query,{session: db_params.session});
		return result;
	}
}
